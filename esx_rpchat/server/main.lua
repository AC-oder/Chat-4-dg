...
-- end
 ...
 RegisterCommand('do', function(source, args, rawCommand)
    ...
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(3, 194, 0, 0.6); border-radius: 3px;"><i class="fab fa-user"></i> {0}:<br> {1}</div>',
        args = { playerName, msg }
    })
end, false)
 ...
 RegisterCommand('news', function(source, args, rawCommand)
    ...
    -- fal = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(0, 60, 255, 0.49); border-radius: 3px;"><i class="fas fa-newspaper"></i> News | {0}:<br> {1}<br></div>',
        args = { playerName, msg }
    })
end, false)

 RegisterCommand('anonnews', function(source, args, rawCommand)
    local playerName = GetPlayerName(source)
    local msg = rawCommand:sub(10)
    -- local name = getIdentity(source)
    -- fal = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(0, 60, 255, 0.49); border-radius: 3px;"><i class="fas fa-newspaper"></i> News:<br> {1}<br></div>',
        args = { fal, msg }
    })
end, false)

 RegisterCommand('000', function(source, args, rawCommand)
    local playerName = GetPlayerName(source)
    local msg = rawCommand:sub(4)
    -- local name = getIdentity(source)
    -- fal = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(255, 0, 0, 0.49); border-radius: 3px;"><i class="fas fa-phone"></i> 911 Alert:<br> {1}<br></div>',
        args = { fal, msg }
    })
end, false)
